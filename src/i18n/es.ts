export const navLinks = [
  { href: "#about", label: "Sobre mí" },
  { href: "#projects", label: "Proyectos" },
  { href: "#certifications", label: "Certificaciones" },
  { href: "#experience", label: "Experiencia" },
];

export const heroData = {
  badge: "Desarrollador Frontend",
  h1_1: "Convirtiendo",
  h1_span1: "ideas",
  h1_2: "en web modernas",
  h1_span2: "experiencias.",
  intro_1: "Hola, soy",
  intro_bold1: "Alejandro Guzmán",
  intro_2:
    ". Construyo aplicaciones web escalables y de alto rendimiento usando React y TypeScript.",
  follow_title: "Sígueme:",
  exp_time: "5+",
  exp_span: "Meses de Exp.",
  exp_available: "Disponible para trabajar",
  tech_title: "Tecnologías con las que trabajo",
};

export const buttonText = {
  contact: "Contáctame",
  downloadCv: "Descargar CV",
  sendMessage: "Enviar Mensaje",
  sendingMessage: "Enviando",
};

export const aboutData = {
  badge: "Sobre mí",
  h2_1: "Construyendo el futuro,",
  h2_span: "un componente a la vez.",
  p1_1: "Soy un Desarrollador Frontend con formación en",
  p1_span: "Ingeniería en Electrónica y Telecomunicaciones",
  p1_2:
    ", apasionado por el desarrollo web y las experiencias digitales modernas.",
  p2: "Disfruto construyendo interfaces responsivas e interactivas usando tecnologías como React, TypeScript, JavaScript y Tailwind CSS, con un fuerte enfoque en rendimiento, usabilidad y código limpio.",
  p3: "También me fascinan los avances en inteligencia artificial y disfruto explorar cómo estas tecnologías pueden ampliar mis conocimientos y mejorar la forma en que construimos soluciones digitales.",
  mission:
    "Mi misión es crecer continuamente como desarrollador, crear aplicaciones web de alta calidad y evolucionar hacia convertirme en un Desarrollador Full Stack.",
};

export const projectData = {
  badge: "Trabajo Destacado",
  h2_1: "Proyectos que",
  h2_span: "generan impacto",
  desc: "Una selección de mi trabajo reciente, desde aplicaciones web complejas hasta herramientas innovadoras que resuelven problemas reales.",
};

export const certificationData = {
  badge: "Certificaciones & Cursos",
  h2_1: "Compromiso con el",
  h2_span: "Aprendizaje Continuo",
};

export const experienceData = {
  badge: "Trayectoria Profesional",
  h2_1: "Experiencia que",
  h2_span: "habla por sí sola.",
};

export const contactData = {
  badge: "Ponte en Contacto",
  h2_1: "Construyamos",
  h2_span: "algo increíble",
  desc: "¿Tienes un proyecto en mente? Me encantaría escucharlo. Envíame un mensaje y hablemos sobre cómo podemos trabajar juntos.",
  form_name: "Nombre",
  form_name__placeholder: "Tu nombre ...",
  form_name__required: "El nombre es obligatorio",
  form_email: "Correo",
  form_email_placeholder: "tu.email@ejemplo.com",
  form_email__required: "El correo es obligatorio",
  form_email__invalid: "Correo inválido",
  from_message: "Hola, estoy interesado en...",
  contact_title: "Información de Contacto",
  form_message__required: "¡El mensaje es obligatorio!",
  available_title: "Actualmente Disponible",
  available_desc:
    "Estoy abierto a nuevas oportunidades y proyectos emocionantes. ¡Cuéntame qué necesitas, hablemos!",
};

export const footerData = {
  copyright: "© Alejandro Guzmán Rodríguez | Todos los derechos reservados.",
};

export const footerLinks = [
  { href: "#about", label: "Sobre mí" },
  { href: "#projects", label: "Proyectos" },
  { href: "#certifications", label: "Certificaciones" },
  { href: "#experience", label: "Experiencia" },
  { href: "#contact", label: "Contacto" },
];

export const projects = [
  {
    title: "Beats 3",
    description:
      "Landing page responsiva de producto inspirada en Beats, enfocada en diseño UI moderno, layouts estructurados y presentación clara del producto.",
  },
  {
    title: "GPT-3",
    description:
      "Landing page responsiva inspirada en GPT-3 y plataformas de IA, enfocada en estructura UI limpia, secciones de contenido organizadas y diseño frontend moderno.",
  },
  {
    title: "MacBook Pro",
    description:
      "Landing page interactiva de producto con visualización 3D y animaciones, construida con React, Three.js y GSAP, enfocada en UI moderna, transiciones suaves y diseño responsivo.",
  },
  {
    title: "HooBank",
    description:
      "Landing page fintech responsiva enfocada en diseño UI moderno, componentes estructurados y presentación clara de servicios financieros digitales.",
  },
];

export const highlightsAbout = [
  {
    title: "Código Limpio",
    description:
      "Escribir código mantenible y escalable que resista el paso del tiempo.",
  },
  {
    title: "Rendimiento",
    description:
      "Optimizar la velocidad y entregar experiencias de usuario ultrarrápidas.",
  },
  {
    title: "Colaboración",
    description: "Trabajar de cerca con los equipos para hacer realidad las ideas.",
  },
  {
    title: "Innovación",
    description:
      "Mantenerse a la vanguardia con las últimas tecnologías y mejores prácticas.",
  },
];

export const experiences = [
  {
    role: "Desarrollador Frontend",
    period: "2025-2026 (6 Meses)",
    description: [
      "Desarrollo de diseños web responsivos usando Webflow.",
      "Implementación de soluciones personalizadas con JavaScript.",
      "Resolución de problemas complejos de interfaces e interacciones.",
      "Mejora de rendimiento y compatibilidad entre navegadores.",
      "Aplicación de principios UX para mejorar la usabilidad.",
    ],
  },
];

export const certifications = [
  {
    name: "Iniciación al desarrollo con IA",
    description:
      "Curso enfocado en el desarrollo real con tecnologías de IA y mejores prácticas en Ingeniería de Software con Agentes de IA en el momento actual.",
  },
  {
    name: "TanStack Query",
    description:
      "Enfocado en la obtención eficiente de datos y gestión del estado del servidor en React usando TanStack Query. Cubre estrategias de caché, mutaciones, refetching en segundo plano y sincronización de API. Enfocado en construir aplicaciones escalables y performantes con patrones modernos de manejo de datos.",
  },
  {
    name: "React Router",
    description:
      "Curso enfocado en implementar enrutamiento del lado del cliente en aplicaciones React. Cubre navegación, rutas dinámicas, protección de rutas y estructuración de layouts con React Router. Fortalece habilidades para construir aplicaciones React escalables y bien estructuradas.",
  },
  {
    name: "Solving TypeScript Errors",
    description:
      "Tutorial práctico para identificar y corregir errores comunes de TypeScript. Aprende a interpretar mensajes del compilador y aplicar soluciones de tipos correctas. Desarrolla habilidades de depuración y confianza en proyectos reales de TypeScript.",
  },
  {
    name: "Beginner's TypeScript",
    description:
      "Tutorial interactivo que enseña los fundamentos de TypeScript a través de ejercicios prácticos. Cubre el tipado de variables, funciones y errores mientras introduce herramientas modernas. Construye una base sólida para el uso confiable de TypeScript en proyectos reales.",
  },
];

export const testimonials = [
  {
    quote: "AAAAAAipsum dolor sit amet",
    role: "CTO, Tech Innovation Inc.",
  },
];

export const contactInfo = [
  { label: "Correo" },
  { label: "Teléfono" },
  { label: "Ubicación" },
];
